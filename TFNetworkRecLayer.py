...
class RelativePositionalEncodingLayer(_ConcatInputLayer):
  ...
  def __init__(self, n_out, forward_weights_init="glorot_uniform", clipping=16, fixed=False, **kwargs):
    """
    :param n_out: Feature dimension of encoding.
    :param clipping: After which distance to fallback to the last encoding
    :param fixed: Uses sinusoid positional encoding instead of learned parameters
    """
    ...
    self.output.shape = (None, None, n_out)
    self.output.batch_dim_axis = None
